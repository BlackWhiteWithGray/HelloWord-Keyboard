...
public:
    ...
    static const uint16_t RAW_REPORT_SIZE = 0;//1 + 32;
    static const uint16_t MEDIA_REPORT_SIZE = 1 + 2;
    static const uint16_t MOUSE_REPORT_SIZE = 1 + 4;
    static const uint16_t HID_REPORT_SIZE = KEY_REPORT_SIZE + RAW_REPORT_SIZE + MEDIA_REPORT_SIZE + MOUSE_REPORT_SIZE;
    ...
    enum KeyCode_t : int16_t
    {
        ...
        FN = 1000,TP1,TP2,TP3,TP4,TP5,TP6,
    ...
    bool isMediaReport = false;
    bool isEepPush = false;
    bool isFNPress = false;
    bool TouchTestLED = false;
    uint8_t  KeyDelayCnt = 0;
    ...
    uint16_t GetHidReportSize(uint8_t _reportId);
    uint8_t* GetHidReportBuffer(uint8_t _reportId);
    uint8_t GetTouchBarState(uint8_t _id = 0);
    uint8_t TouchBarCount = 0;
    uint8_t TouchBarTime =0;
    void MediaPress(HWKeyboard::KeyCode_t _key);
    void MediaRelease(void);
    void MouseMove(uint8_t move);
    uint8_t TouchBarSlide(void);
    void DeskTop(HWKeyboard::KeyCode_t _key);
    ...
    {ESC,F1,F2,F3,F4,F5,F6,F19,F20,F9,F10,F11,F12,INSERT,
        GRAVE_ACCENT,NUM_1,NUM_2,NUM_3,NUM_4,NUM_5,NUM_6,NUM_7,NUM_8,NUM_9,NUM_0,MINUS,EQUAL,BACKSPACE,MUTE,
        TAB,A,B,C,D,E,F,G,H,I,J,LEFT_U_BRACE,RIGHT_U_BRACE,BACKSLASH,DELETE,
        CAP_LOCK,K,L,M,N,O,P,Q,R,S,SEMI_COLON,QUOTE,ENTER,VOLUME_UP,
        LEFT_SHIFT,T,U,V,W,X,Y,Z,COMMA,PERIOD,SLASH,RIGHT_SHIFT,A,VOLUME_DOWN,
        LEFT_CTRL,LEFT_GUI,LEFT_ALT,SPACE,RIGHT_ALT,FN,RIGHT_CTRL,LEFT_ARROW,DOWN_ARROW,RIGHT_ARROW
        }
    };
